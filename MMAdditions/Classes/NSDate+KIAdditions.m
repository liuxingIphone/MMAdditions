...
- (NSString *)normalizeDateString_home
{
    NSCalendar *gregorian = [[NSCalendar alloc] initWithCalendarIdentifier:NSCalendarIdentifierGregorian];
    NSUInteger unitFlags = NSCalendarUnitYear | NSCalendarUnitMonth | NSCalendarUnitDay | NSCalendarUnitHour | NSCalendarUnitMinute | NSCalendarUnitSecond;
    NSDateComponents *components = [gregorian components:unitFlags fromDate:self toDate:self options:0];
    if ([components year] >= 1) {
        return [NSDate getStringFromDate:self dateFormatter:@"yyyy-MM-dd"];
    } else if ([components day] >= 1) {
        return [NSDate getStringFromDate:self dateFormatter:@"MM-dd"];
    } else if ([components minute] > 0) {
        return [NSDate getStringFromDate:self dateFormatter:@"HH:mm"];
    }else {
        return @"刚刚";
    }
}
